...
simulated function ModeTick(float deltaTime)
{
	...
	local Vector HitLocation, HitNormal, PawnHitLocation, EndEffect;
	local Actor Other;
	local Rotator Aim;
	local LinkGun LinkGun;
	local float Step, ls;
	local Bot B;
	local bool bShouldStop, bNeedRevert, bIsHealingObjective;
	...
	if (myHasAmmo(LinkGun) && ((UpTime > 0.0) || (Instigator.Role < ROLE_Authority)))
	{
		...
		if (Instigator.Role == ROLE_Authority)
		{
			...
			{
				if (
					IsLinkable(B.Squad.SquadLeader.Pawn)
