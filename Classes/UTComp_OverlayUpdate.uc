...
const f_UPDATE_TIME = 1.0;
const iMAXPLAYERS = 8;

function InitializeOverlay()
{
	if (Level.Game.bTeamGame)
		SetTimer(f_UPDATE_TIME, true);
}

function Timer()
{
	if (UTCompMutator.bTeamOverlay)
		UpdateTeamOverlayInfo();
	else if (!bVariablesCleared)
		ClearTeamOverlayInfo();
	
	if (UTCompMutator.bPickupsOverlay)
		UpdatePowerupsInfo();
...
function bool IsRelevant(UTComp_PRI uPRI, PlayerReplicationInfo PRI, PlayerReplicationInfo PRIOverlay)
{
	...
	if (PRI.Team != None && PRIOverlay.Team != None && (PRI.Team == PRIOverlay.Team || uPRI.GetTeamX == PRIOverlay.Team.TeamIndex))
		return true;
}
...
function FindInfoForTeam(byte iTeam)
{
	...
	for(C = Level.ControllerList; C != None; C = C.NextController)
	{
		...
		if (uPRI != None && (C.GetTeamNum() == iTeam || iTeam == uPRI.GetTeamX || (uPRI.GetTeamX == 255 && C.PlayerReplicationInfo.bOnlySpectator)))
		{
			k = 0;
			for(i = 0; i < iMAXPLAYERS; i++)
			{
		...
		else if (uPRI != None && uPRI.GetTeamX != 255 && uPRI.GetTeamX != iTeam)
		{
			for(i = 0; i < iMAXPLAYERS; i++)
			{
				if (iTeam == 0)
