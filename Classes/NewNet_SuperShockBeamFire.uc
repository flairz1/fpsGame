...
simulated function SpawnClientBeamEffect(Vector Start, Rotator Dir, Vector HitLocation, Vector HitNormal, int ReflectNum)
{
	...
	if (BS_xPlayer(C) != None)
		BS_xPlayer(C).SendWeaponEffect(class'UTComp_SuperShockRifleEffect', Instigator, Start, vector(Dir), HitLocation, HitNormal, ReflectNum);
}
